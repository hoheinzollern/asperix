...
// all terms in tv are ground RangeTerms
	...
	for (std::vector<Argument*>::iterator it = tuples->begin(); it != tuples->end(); it++){
		switch (st){
		case TRUE_ : 	getPred()->addTrueInstance(**it);
			break;
		case MBT_ : 	getPred()->addMbtInstance(**it);
			break;
...
// all terms in tv are ground RangeTerms
	...
	for (std::vector<Argument*>::iterator it = tuples->begin(); it != tuples->end(); it++){
		switch (st){
		case TRUE_ : 	added = getPred()->addTrueInstance(**it, aq) || added;
			break;
		case MBT_ : 	added = getPred()->addMbtInstance(**it, aq) || added;
			break;
		default :	throw RuntimeExcept("try to add instance with illegal status");
		}
		delete (*it);
