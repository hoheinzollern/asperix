...
// _currentScc is considered solved, propagate this information
  ...
  try {
	...
	changes.recordIntermediaryExtensions(*this, nextScc); // record result of propagation
	return true;
   }
   catch (ContradictoryConclusion&){
...
// and, if return false, stop indicates if back-track is needed
	...
	do{
	   while (found && !propagate(changes)){ // propagation leads to contradictory conclusion
		prohibiteCurrentInstanceOfNDRPbMbtDescenteGauche(changes);
...
bool Program::answerSetSearch(int n){
	...
	if (existsSupportedUnblockedRule(rset)) 
		return false; // contrainte violée, le prg est UNSAT
	...
	do{
	   ...
	   if (!_chan_bool_stack.empty()){
		...
		if (withRule){ // remontée gauche
			...
			if (! stop ){ // search for an AS without _current_non_definite_rule
				_bad_choice++;
